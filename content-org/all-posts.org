...
In this I will describe the Network setup for my Threathunting Environment,
More about how I build the honeypots and the ELK Server I will describe in this
series about threathunting.
...
The Honeypot and ELK Server are Placed inside the 210 VLAN which is isolated
from the rest network. The Honeypot gets firewalled completely except for
traffic coming from the outside to port 22, which is then redirected to port
2222 on the honeypot. The Honeypot ingests data via filebeat into the ELK Server.
